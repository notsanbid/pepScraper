...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    while (T-- > 0) {
      int n = scn.nextInt();
      String s = scn.next();
      SuffixArray sf = new SuffixArray(s + s);
      int i = 0;
      int ans = -1;

      while (i < sf.n) {      // main idea is to get the first whose i<n
        if (sf.SA[i] < n) {   // and is the minimum if suffix is repeated
          ans = sf.SA[i];
          break;
        }
        i++;
      ...
      while (i < sf.n && sf.lcp[i - 1] >= n) {
        ans = Math.min(ans, sf.SA[i]);
        // System.out.println(i++);
        i++;
      }
      // s+=s;
      // System.out.println(s.substring(ans,ans+n));
