...
public class Main {
  public static void main(String args[]) {
    ...
    while (T-- > 0) {
      ...
      st.build();
      int q = scn.nextInt();
      HashSet<String> good = new HashSet<String>();   // just to store a record
      HashSet<String> bad = new HashSet<String>();    // juat to store a record
      ...
      while (q-- > 0) {
        ...
        if (bad.contains(pat)) {
          ...
          continue;
        } else if (good.contains(pat)) {
          System.out.println("y");
          continue;
        }
        boolean occurances = st.findPattern(st.root, pat, 0);
        if (occurances) {
          System.out.println("y");
          good.add(pat);
        } else {
...
class SuffixTree {
  ...
  // there are 3 phases
  // phase 1: got stuck in suffix tree, i.e. next char in st != next in pattern   result: pattern not found return empty arraylist
  // phase 2: we run out of pattern,                                              result: pattern found, get to every leaf node from here and get index[node]
  // phase 3: we run out of tree,i.e we reach leaf node,                          result: pattern not found return empty arraylist

  public boolean findPattern(SuffixNode root, String pat, int i) {
    // ArrayList<Integer> ret = new ArrayList<>();
    if (root == null) {
      return false;
    }
    int j = root.start;
    while (j <= root.end.end) {
      if (i == pat.length() - 1 && pat.charAt(i) == input[j]) return true; // ran out of pattern
      if (pat.charAt(i) != input[j]) {                // pattern not found
        return false;
      }
      i++;
      j++;
    ...
    for (SuffixNode node : root.child) {
      boolean ans = findPattern(node, pat, i);
      if (ans) return ans;
    }
    return false;
  }

  public ArrayList<Integer> leafCount(SuffixNode root) {
    if (root == null) {
      ArrayList<Integer> ret = new ArrayList<>();
      return ret;
    }
    ArrayList<Integer> myAns = new ArrayList<Integer>();
    if (root.index != -1) {                     // index != 1 only for leaf nodes
      myAns.add(root.index);
    }
    for (SuffixNode node : root.child) {
      ArrayList<Integer> nextAns = leafCount(node);
      for (int x : nextAns) myAns.add(x);
    }
    return myAns;
  }
  ///////////////////////////////////////////////////////////////// Code ends here
