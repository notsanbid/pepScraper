...
public class Main {
  public static void main(String args[]) {
    ...
    // prints the start indexes of occurances of pattern
    String pat = scn.next();

    ArrayList<Integer> occurances = st.findPattern(st.root, pat, 0);
    Collections.sort(occurances);
    for (int x : occurances) System.out.println(x);
  }
}
class SuffixTree {
  ///////////////////////////////////////////////////////////////// Code starts here
  ...
  public ArrayList<Integer> findPattern(SuffixNode root, String pat, int i) {
    ...
    while (j <= root.end.end) {
      if (i == pat.length() - 1 && pat.charAt(i) == input[j]) return leafCount(root); // ran out of pattern
    ...
    for (SuffixNode node : root.child) {
      ArrayList<Integer> ans = findPattern(node, pat, i);
