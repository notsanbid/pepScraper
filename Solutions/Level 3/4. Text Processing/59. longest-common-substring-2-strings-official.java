...
public class Main {
  ...
  static class SuffixArray {
    ...
    int MAX_N = 2000100;    // take care of this!
    int n = -1;
    String s;
  ...
  public static boolean areApart(int mid, int a, int b) {
    if (b > a) {
      int temp = a;
      a = b;
      b = temp;
  ...
  public static void main(String[] args) {
    ...
    SuffixArray sf = new SuffixArray(s1 + '@' + s2);

    int maxi = 0;
    int maxl = 0;

    for (int i = 0; i < sf.n; i++) {
      if (sf.lcp[i] > maxl && areApart(N, sf.SA[i], sf.SA[i + 1])) {
        maxl = sf.lcp[i];
        maxi = Math.min(sf.SA[i], sf.SA[i + 1]);
      }
    }
    System.out.print(s1.substring(maxi, maxi + maxl));
    scn.close();
  }
}