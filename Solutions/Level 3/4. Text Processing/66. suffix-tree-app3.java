...
class Main {
  public static void main(String args[]) {
    ...
    String concatinated = s1 + "#" + s2;
    SuffixTree st = new SuffixTree(concatinated.toCharArray());
    st.build();
    // st.dfsTraversal();

    st.printLCS(s1.length());
...
class SuffixTree {
  ///////////////////////////////////////////////////////////////// Code starts here
  int mid = -1;
  ArrayList<SuffixNode> allBigs = new ArrayList<SuffixNode>();
  void printLCS(int len1) {
    mid = len1;
    int[] maxi = new int[1];

    for (SuffixNode cc : root.child) {
      dfsNodeFill(maxi, 0, cc);
    }
    System.out.println(maxi[0]);
    ArrayList<Integer> sp = new ArrayList<Integer>();
    for (SuffixNode cn : allBigs) {
      dfsLeafPrint(sp, cn);
    }
    // System.out.println(allBigs.size());
    for (int x : sp) System.out.print(x + " ");
    System.out.println();
  }

  void dfsLeafPrint(ArrayList<Integer> sp, SuffixNode curr) {
    if (curr == null) return;
    int cc = 0;
    for (SuffixNode child : curr.child) {
      if (child != null) cc++;
    }
    if (cc == 0) {
      sp.add(curr.index);
    ...
    for (SuffixNode child : curr.child) {
      dfsLeafPrint(sp, child);
    }
  }

  boolean[] dfsNodeFill(int[] maxi, int clen, SuffixNode node) {
    boolean[] myAns = new boolean[2];
    if (node == null) return myAns;

    int cc = 0;
    for (SuffixNode cchild : node.child) {
      if (cchild != null) cc++;
    }
    if (cc == 0) {
      if (node.index < mid) myAns[0] = true;
      else if (node.index > mid) myAns[1] = true;
      return myAns;
    }
    ...
    for (SuffixNode child : node.child) {
      boolean[] nextAns = dfsNodeFill(maxi, clen, child);
      myAns[0] = myAns[0] | nextAns[0];
      myAns[1] = myAns[1] | nextAns[1];
    }

    if (myAns[0] && myAns[1] && clen >= maxi[0]) {
      if (clen > maxi[0]) allBigs = new ArrayList<SuffixNode>();
      maxi[0] = clen;
      allBigs.add(node);
    }
    return myAns;
