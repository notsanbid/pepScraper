...
public class Main {
  ...
  public static void main(String args[]) {
    ...
    // get deepest length node which is not a leaf and print all occurances
    st.dfsFiller(st.root, 0, maxLen);

  }
}
class SuffixTree {
  ///////////////////////////////////////////////////////////////// Code starts here
  void dfsLeafer(ArrayList<Integer> sp, SuffixNode cn) {
    if (cn == null) return;
    int ccount = 0;
    for (SuffixNode cc : cn.child) if (cc != null) ccount++;
    if (ccount == 0) {
      sp.add(cn.index);
      return;
    }
    for (SuffixNode cc : cn.child) {
      dfsLeafer(sp, cc);
    }
  }
  ArrayList<SuffixNode> maxList = new ArrayList<SuffixNode>();
  void dfsFiller(SuffixNode node, int clen, int[] maxLen) {
    if (node == null) return;
    if (node.index != -1) return;
    // System.out.println(node.index+" "+clen+" "+node.start+" "+node.end.end);
    clen = clen + (node != root ? node.end.end - node.start + 1 : 0);
    if (clen >= maxLen[0]) {
      if (clen > maxLen[0]) maxList = new ArrayList<SuffixNode>();
      maxList.add(node);
      maxLen[0] = clen;
    }
    ...
    for (SuffixNode cchild : node.child) {
      ...
      dfsFiller(cchild, clen, maxLen);
    }
    if (node == root) {
      System.out.println(maxLen[0]);
      ArrayList<Integer> sp = new ArrayList<Integer>();
      for (SuffixNode co : maxList) {
        dfsLeafer(sp, co);
      }
      Collections.sort(sp);
