...
//   static OutputWriter out = new OutputWriter(System.out);
  ...
  */
  ...
  static int bs(int ar[], int k) {

    int low = 0;
    int high = ar.length - 1;
    if (ar[high] <= k)return high + 1;
  ...
  static int query(int node, int start, int end, int l, int r, int k) {
    ...
    if (l <= start && end <= r) {
      int bi = bs(tree[node], k);

      return tree[node].length - bi;
    } else {
      int mid = (start + end) / 2;
      int la = query(node * 2, start, mid, l, r, k);
      int ra = query(node * 2 + 1, mid + 1, end, l, r, k);
      return la + ra;
    }
  }
